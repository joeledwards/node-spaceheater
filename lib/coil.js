...
async function coil () {
  ...
  const id = await idResolved.promise

  while (isRunning()) {
    await flame({ id, cyclone, metrics, isRunning })
...
async function flame ({ id, cyclone, metrics, isRunning }) {
  ...
  while (isRunning() && elapsed() <= haltCheckFrequency) {
    ...
    metrics.add(`coil.flames.${id}.cycles`)
  }
